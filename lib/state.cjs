...
function normalizeState(value, sessionId, now = Date.now()) {
  ...
  state.recoveryTurns = Array.isArray(value.recoveryTurns) ? value.recoveryTurns.filter((turn) => typeof turn === 'string') : [];
  state.recoveredAt = value.recoveredAt && typeof value.recoveredAt === 'object' ? value.recoveredAt : null;
...
function recordCheck(state, { turnId, verdict, answers }, now = Date.now()) {
  ...
  const previous = turnId ? state.checkHistory.find((entry) => entry.turnId === turnId) : null;
  const historyEntry = {
    turnId: turnId || null,
    ...
    cutoffConcrete: verdict.detail?.cutoffConcrete === true,
  ...
  if (previous) Object.assign(previous, historyEntry);
  else state.checkHistory.push(historyEntry);
  ...
  if (state.status !== 'degraded_approved') {
    if (verdict.pause) {
      state.status = 'degraded';
      state.recoveryTurns = [];
    } else if (state.status === 'degraded') {
      if (verdict.tibo !== 'pass') state.recoveryTurns = [];
      else if (turnId && !previous) {
        state.recoveryTurns = [...(state.recoveryTurns || []), turnId];
        const required = recoveryPasses();
        if (state.recoveryTurns.length >= required) {
          state.status = 'healthy';
          state.recoveredAt = { reason: 'consecutive_tibo_pass', at: now, required, turnIds: [...state.recoveryTurns] };
          state.recoveryTurns = [];
        }
      }
...
function recoveryPasses() {
  const value = Number(process.env.MODEL_DEGRADATION_GUARD_RECOVERY_PASSES);
...
function startCheck(state, { turnId, token }, now = Date.now()) {
  // A turn without a qualifying check breaks a recovery streak.
  if (state.status === 'degraded' && state.check && state.check.turnId !== turnId
    && !(state.checkHistory || []).some((entry) => entry.turnId === state.check.turnId)) {
    state.recoveryTurns = [];
  }
...
// 有 turn_id 时必须同时匹配 token 与回合归属；Codex 未提供 turn_id 时退回 token 口径，
// 否则打卡写回来的答案永远读不到，写/删会被永久拦住。
function answersForCurrentCheck(state, turnId) {
  if (!state || !state.check || !state.answers) return null;
  if (state.answers.token !== state.check.token) return null;
  if (typeof turnId !== 'string' || !turnId) return state.answers;
  return state.answers.turnId === turnId && state.check.turnId === turnId ? state.answers : null;
}
